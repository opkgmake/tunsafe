...
WorkerLoop::WorkerLoop() {
  ...
  if (pthread_cond_init(&cond_, NULL) != 0)
    tunsafe_die("pthread_cond_init 失败");
}
...
bool UdpLoop::Initialize(int listen_port, WorkerLoop *worker) {
  ...
  if (fd < 0) { RERROR("打开 udp 时出错"); return false; }
...
bool TunLoop::Initialize(char devname[16], WorkerLoop *worker) {
  ...
  if (fd < 0) { RERROR("打开 tun 时出错"); return false; }
  fcntl(fd, F_SETFD, FD_CLOEXEC);
  fd_ = fd;
  worker_ = worker;
  return true;
