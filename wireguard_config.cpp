  ...
  if (strcmp(group, "[Interface]") == 0) {
    ...
    if (wg_->dev().plugin() && wg_->dev().plugin()->OnUnknownInterfaceSetting(key, value)) {
      ...
      if (ParseBoolean(value, &flag)) {
        ...
        RERROR("ObfuscateTCP 中的未知模式: %s", value);
      }
    ...
    if (key == NULL) {
      if (!had_interface_) {
        RERROR("[Interface] 缺少 PrivateKey.");
        return false;
      }
      FinishGroup();
    ...
    if (wg_->dev().plugin() && wg_->dev().plugin()->OnUnknownPeerSetting(peer_, key, value)) {
      ...
      if (!ParseSockaddrInWithPort(value, &sin, dns_resolver_))
        return false;
      peer_->SetEndpoint(proto, sin, hostname);
    ...
    if (buf[0] == '[') {
      ...
      *sepe = 0;
      ...
      if (!file_parser.ParseFlag(group, buf, sep)) {
        RERROR("解析错误 %s.%s = %s", group, buf, sep);
        return false;
      }
    }
    if (!nl)
    ...
    if (strcmp(key, "get") == 0) {
      ...
      if (strcmp(key, "private_key") == 0) {
        ...
        if (strcmp(key, "remove") == 0) {
          ...
          const char* hostname = value;
          std::string modified_hostname(hostname); 
          if (strncmp(modified_hostname.c_str(), "tcp://", 6) == 0) {
        	modified_hostname = modified_hostname.substr(6); 
          }
          if (!ParseSockaddrInWithPort(value, &sin, NULL)) goto getout_fail;
          peer->SetEndpoint(kPacketProtocolUdp, sin, modified_hostname.c_str());
