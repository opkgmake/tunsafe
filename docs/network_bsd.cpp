...
bool UdpSocketBsd::Initialize(int listen_port) {
  ...
  int udp_fd = socket(AF_INET6, SOCK_DGRAM, 0); // 修改为AF_INET6
  if (udp_fd < 0) {
    RERROR("socket(SOCK_DGRAM) failed");
    return false;
  }
  sockaddr_in6 sin = {0}; // 修改为sockaddr_in6
  sin.sin6_family = AF_INET6; // 修改为AF_INET6
  sin.sin6_port = htons(listen_port); // 修改为sin6_port
...
bool UdpSocketBsd::DoRead() {
  ...
  sin_len = sizeof(read_packet->addr.sin6); // 修改为sin6
  int r = recvfrom(fd_, read_packet->data, kPacketCapacity, 0,
                   (sockaddr*)&read_packet->addr.sin6, &sin_len); // 修改为sin6
  if (r >= 0) {
...
bool UdpSocketBsd::DoWrite() {
  ...
  int r = sendto(fd_, udp_queue_->data, udp_queue_->size, 0,
                 (sockaddr*)&udp_queue_->addr.sin6, sizeof(udp_queue_->addr.sin6)); // 修改为sin6
  if (r < 0) {
    if (errno == EAGAIN) {
      udp_writable_ = false;
      SetPollFlags(POLLIN | POLLOUT);
