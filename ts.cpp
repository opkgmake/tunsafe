  ...
  *(uint32*)temp = (uint32)(data_size + 1);
  ...
  do {
    if (!WriteFile(pipe, temp + pos, bytes_to_write, &bytes_written, NULL)) {
      fprintf(stderr, "写入pipe服务时出错，错误 = %d\n", GetLastError());
      break;
    }
    pos += bytes_written;
...
static bool ReadMessageFromService(HANDLE pipe, int *message, std::string *data) {
  ...
  if (!ReadExactBytesFromPipe(pipe, header, 5) || (message_size = *(uint32*)header) == 0) {
    fprintf(stderr, "从pipe服务读取错误，错误 = %d\n", GetLastError());
    return false;
  }
  *message = header[4];
...
static HANDLE ConnectToService(const char *devname, bool want_updates, bool want_create = false) {
  ...
  for (;;) {
    ...
    if (error != ERROR_PIPE_BUSY) {
      fprintf(stderr, "连接到 TunSafe 服务时出错，错误 = %d\n", error);
      if (error == ERROR_FILE_NOT_FOUND)
        fprintf(stderr, "请检查TunSafe服务是否启动\n");
      return NULL;
    }
    if (!WaitNamedPipe(TUNSAFE_PIPE_NAME, 10000)) {
      fprintf(stderr, "连接到 TunSafe 服务时出错，超时.\n");
      return NULL;
    }
  }
}
  ...
  if (pipe != NULL &&
    ...
    if (message_code == TS_SERVICE_REQ_TEXT_PROTOCOL_REPLY) {
      ...
      if (message_code == TS_SERVICE_MSG_ERROR_REPLY) {
        ...
        fprintf(stderr, "来自 TunSafe 服务的未知回复 (%d) .\n", message_code);
      }
    }
  }
  CloseHandle(pipe);
...
static bool GetInterfaceList(std::string *result) {
  ...
  if (pipe != NULL &&
    ...
    if (message_code == TS_SERVICE_REQ_GETINTERFACES_REPLY) {
      ...
      fprintf(stderr, "GetInterfaceList: 错误回复\n");
    }
  }
  CloseHandle(pipe);
  return rv;
...
static int OpenUserspaceInterface(const char *iface) {
  ...
  if (strchr(iface, '/') != NULL) {
    fprintf(stderr, "无法打开用户模式套接字：没有这样的设备\n");
    goto getout;
  }
  ...
  if (connect(fd, (struct sockaddr *)&un, sizeof(un)) < 0) {
    if (errno == ECONNREFUSED)
      ...
      perror("打开wireguard 用户模式接口套接字时出错");
    goto getout;
  }
  return fd;
...
static int HandleStopCommand(int argc, char **argv) {
  if (argc != 1 || (argc > 0 && strcmp(argv[0], "--help") == 0)) {
    fprintf(stderr, "\n\033[1;31m用法： \033[0m " EXENAME " stop <接口名>\n\n");
    return 1;
  }
...
void ShowHelp() {
  fprintf(stderr,
    "\n\033[1;31m用法： \033[0m " EXENAME " <命令> [<参数>]\n\n"
#if defined(OS_POSIX)
    "        " EXENAME " 文件名.conf\n\n"
#endif  // defined(OS_POSIX)
    "可用的子命令:\n\n"
    ...
    "  genkey: 输出新的私钥\n"
    "  genpsk: 输出新的预共享密钥\n"
    "  pubkey: 从输入读取私钥，并输出其公钥\n\n"
    "要查看有关子命令的更多帮助，请使用 子命令 --help\n\n");
}
...
static int HandleShowCommand(int argc, char **argv) {
  if (argc != 0 && strcmp(argv[0], "--help") == 0) {
    fprintf(stderr, "\n\033[1;31m用法： \033[0m " EXENAME " show { <接口> | all | interfaces }\n\n");
    return 0;
  }
...
#include <cstring>
#include <cstdint>
bool IsFilePath(const char *s) {
  // 检查传入的参数是否是文件路径
  // 这里简单地假设如果参数中包含文件路径的分隔符（比如'/'）且没有等号，就认为是文件路径
  return (strchr(s, '/') != nullptr && strchr(s, '=') == nullptr);
}

bool ConvertBase64KeyToHex(const char *s, char key[65]) {
  if (IsFilePath(s)) {
    // 如果是文件路径，则读取文件内容
    ...
    if (file) {
      ...
      uint8_t tmp[32];
      size_t size = 32;
      if (!base64_decode((uint8_t*)fileContent.c_str(), fileContent.size(), tmp, &size) || size != 32) {
        return false;
      }
    ...
    uint8_t tmp[32];
    size_t size = 32;
    if (!base64_decode((uint8_t*)s, strlen(s), tmp, &size) || size != 32) {
      return false;
    }
    PrintHexString(tmp, 32, key);
    return true;
...
static int HandleSetCommand(int argc, char **argv) {
  ...
  if (argc == 0 || (argc > 0 && strcmp(argv[0], "--help") == 0)) {
    fprintf(stderr, "\n\033[1;31m用法： \033[0m " EXENAME " set <接口名> address <ip地址> listen-port <端口> private-key <私钥文件路径> "
            "peer <base64 公钥> [remove] preshared-key <共享密钥文件路径> endpoint <ip地址>:<端口> "
            "persistent-keepalive <间隔秒数> allowed-ips <ip地址1>/<cidr地址1>,<ip地址2>/<cidr地址2> \n\n其中 remove 为删除参数命令\n\n");
    return 1;
  }
...
static int HandleStartCommand(int argc, char **argv) {
  if (argc < 1 || argc > 2 || strcmp(argv[0], "--help") == 0) {
    fprintf(stderr, "\n\033[1;31m用法： \033[0m " EXENAME " start <接口名> [<文件名>]\n\n");
    return 1;
  }
...
int HandleCommandLine(int argc, char **argv, CommandLineOutput *output) {
  ...
  if (!strcmp(subcommand, "show")) {
    ...
    if (argc != 0) {
      fprintf(stderr, "\n\033[1;31m用法： \033[0m " EXENAME " log\n");
      return 1;
    }
...
start_usage:
      fprintf(stderr, "\n\033[1;31m用法： \033[0m " EXENAME " start [-d/--daemon] [-n <接口名>] [<文件名>]\n\n");
      return 0;
    }
    for (; argc; argc--, argv++) {
      char *arg = argv[0];
    ...
    if (argc != 0 || (argc > 0 && strcmp(argv[0], "--help") == 0)) {
      fprintf(stderr, "\n\033[1;31m用法： \033[0m " EXENAME " genkey\n\n");
      return 1;
    }
    ...
    if (argc != 0 || (argc > 0 && strcmp(argv[0], "--help") == 0)) {
      fprintf(stderr, "\n\033[1;31m用法： \033[0m " EXENAME " genpsk\n\n");
      return 1;
    }
    OsGetRandomBytes(key, 32);
    ansi_printf("%s\n", base64_encode(key, 32, base64buf, sizeof(base64buf), NULL));
    ...
    if (n < sizeof(base64) - 2 || n >= sizeof(base64) || 
      ...
      fprintf(stderr, EXENAME ": 密钥格式不正确\n");
      return 1;
    }
    size_t size = 32;
    ...
    ansi_printf("%s\n", base64_encode(key, 32, base64buf, sizeof(base64buf), NULL));
  } else if (!strcmp(subcommand, "--help") || !strcmp(subcommand, "-h") || !strcmp(subcommand, "-help") || !strcmp(subcommand, "help")) {
    ShowHelp();
  } else if (!strcmp(subcommand, "--version") || !strcmp(subcommand, "-v") || !strcmp(subcommand, "-version") || !strcmp(subcommand, "version")) {
