...
bool UdpSocketBsd::Initialize(int listen_port) {
  ...
  int udp_fd = socket(AF_INET6, SOCK_DGRAM, 0); // Change to AF_INET6 for IPv6 support
  if (udp_fd < 0) {
    RERROR("socket(SOCK_DGRAM) 失败");  
    return false;
  }
  int opt = 1;
  if (setsockopt(udp_fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) < 0) {
    close(udp_fd);
    RERROR("setsockopt(SO_REUSEADDR) 失败");
    return false;
  }
  struct sockaddr_in6 sin = {0}; // Use sockaddr_in6 for IPv6
  sin.sin6_family = AF_INET6;
  sin.sin6_port = htons(listen_port);
  if (bind(udp_fd, (struct sockaddr*)&sin, sizeof(sin)) != 0) {
    close(udp_fd);
    RERROR("绑定到 udp 套接字端口 %d 失败", listen_port);
    return false;
  }
...
bool UdpSocketBsd::DoRead() {
  ...
  sin_len = sizeof(read_packet->addr.sin6); // Use sin6 for IPv6
  int r = recvfrom(fd_, read_packet->data, kPacketCapacity, 0,
                   (sockaddr*)&read_packet->addr.sin6, &sin_len); // Use sin6 for IPv6
  if (r >= 0) {
...
bool UdpSocketBsd::DoWrite() {
  ...
  int r = sendto(fd_, udp_queue_->data, udp_queue_->size, 0,
                 (sockaddr*)&udp_queue_->addr.sin6, sizeof(udp_queue_->addr.sin6)); // Use sin6 for IPv6
  if (r < 0) {
    if (errno == EAGAIN) {
      udp_writable_ = false;
      SetPollFlags(POLLIN | POLLOUT);
...
bool UnixDomainSocketListenerBsd::Initialize(const char *devname) {
  ...
  if (bind(fd, (struct sockaddr*)&un_addr_, sizeof(un_addr_)) == -1) {
    RERROR("绑定 unix 域套接字时出错");
    close(fd);
    return false;
  }
  if (listen(fd, 5) == -1) {
    RERROR("绑定 unix 域解决时出错");
    close(fd);
    return false;
  }
  InitPollSlot(fd, POLLIN);
...
bool TcpSocketListenerBsd::Initialize(int port) {
  ...
  if (fd < 0) { RERROR("监听套接字错误"); return false; }
  fcntl(fd, F_SETFD, FD_CLOEXEC);
  fcntl(fd, F_SETFL, O_NONBLOCK);
 ...
 // setsockopt(fd, SOL_SOCKET, SO_REUSEPORT, &optval, sizeof(optval));
  ...
  RINFO("在端口 %d 上启动 TCP 侦听套接字", port);
  InitPollSlot(fd, POLLIN);
  return true;
}
...
bool TcpSocketBsd::InitializeOutgoing(const IpAddr &addr) {
  ...
  if (fd < 0) { perror("套接字：传出 TCP"); return false; }
  fcntl(fd, F_SETFD, FD_CLOEXEC);
  fcntl(fd, F_SETFL, O_NONBLOCK);

  char buf[kSizeOfAddress];
  RINFO("连接到 tcp://%s:%d...", PrintIpAddr(endpoint_, buf), ReadBE16(&endpoint_.sin.sin_port));
...
void TcpSocketBsd::DoRead() {
  ...
  if (bytes_read < 0) {
    if (errno != EAGAIN) {
      RERROR("tcp readv 报告错误代码: %d", errno);
      CloseSocketAndDestroy();
    }
    return;
  }
...
void TcpSocketBsd::DoWrite() {
  ...
  if (n < 0) {
    if (errno != EAGAIN) {
      RERROR("TCP writev 报告错误码: %d", errno);
      CloseSocketAndDestroy();
    } else {
      writable_ = false;
      SetPollFlags(POLLIN | POLLOUT);
