...
int open_tun(char *devname, size_t devname_size) {
  ...
  for(int i = 0; i < 256; i++) {
    ...
    if (fd < 0) {
      RERROR("套接字(SYSPROTO_CONTROL) 失败");
      return -1;
    }

    if (ioctl(fd, CTLIOCGINFO, &ctlinfo) == -1) {
      RERROR("ioctl(CTLIOCGINFO) 失败: %d", errno);
      close(fd);
      return -1;
    }
    ...
    if (connect(fd, (struct sockaddr *)&sc, sizeof(sc)) == 0) {
      ...
      if (getsockopt(fd, SYSPROTO_CONTROL, UTUN_OPT_IFNAME, devname, &devname_size2)) {
        RERROR("getsockopt(UTUN_OPT_IFNAME) 失败");
        close(fd);
        return -1;
      }
...
static void AddOrRemoveRoute(const RouteInfo &cd, bool remove) {
  ...
  if (cd.family == AF_INET) {
    RunCommand("route -q %s %s %s", cmd, buf1, buf2);
  } else {
    RunCommand("route -q %s -inet6 %s %s", cmd, buf1, buf2);
  }
#endif
}
...
bool TunsafeBackendBsd::Configure(const TunConfig &&config, TunConfigOut *out) override {
  ...
  for(const WgCidrAddr &a : config.addresses) {
    if (a.size == 32) {
      RunCommand("ifconfig %s inet %s %s add", devname_, print_ip_prefix(buf, AF_INET, a.addr, a.cidr), print_ip_prefix(buf2, AF_INET, a.addr, -1));
    } else {
      RunCommand("ifconfig %s inet6 %s add", devname_, print_ip_prefix(buf, AF_INET6, a.addr, a.cidr));
    }
  }
  RunCommand("ifconfig %s mtu %d up", devname_, config.mtu);
#endif
  ...
  for (auto it = config.excluded_routes.begin(); it != config.excluded_routes.end(); ++it) {
    if (it->size == 32) {
      ...
      RERROR("不支持 default_route_endpoint_v6");
      return false;
    }
  }
...
void TunsafeBackendBsd::CleanupRoutes() {
  ...
  for(const WgCidrAddr &a : addresses_to_remove_) {
    if (a.size == 32) {
      RunCommand("ifconfig %s inet %s -alias", devname_, print_ip_prefix(buf, AF_INET, a.addr, -1));
    } else {
      RunCommand("ifconfig %s inet6 %s -alias", devname_, print_ip_prefix(buf, AF_INET6, a.addr, -1));
    }
  }
#endif
...
static bool RunOneCommand(const std::string &cmd) {
  ...
  if (exit_code) {
    RERROR("执行失败 (%d) : %s", exit_code, cmd.c_str());
    return false;
  }
  return true;
}
...
void SignalCatcher::SigInt(int sig) {
  if (did_ctrlc)
    exit(1);
  did_ctrlc = true;
  write(1, "检测到 Ctrl+C 退出.再按一次强制退出.\n", sizeof("检测到 Ctrl+C 退出.再按一次强制退出.\n") - 1);
...
SignalCatcher::SignalCatcher(bool *exit_flag, bool *sigalarm_flag) {
...

  {
    ...
    if (timer_create(CLOCK_MONOTONIC, &sev, &timer_id) < 0) {
      RERROR("timer_create 失败");
      return;
    }

    if (timer_settime(timer_id, 0, &tv, NULL) < 0) {
      RERROR("timer_settime 失败");
      return;
    }
  }
...
void TunsafeBackendBsdImpl::OnRequestToken(WgPeer *peer, uint32 type) {
  if (!g_daemon_mode) {
    fprintf(stderr, "登录需要两步验证. 请输入您的身份验证器的值.\nToken: ");
    char buf[100], *rv;
