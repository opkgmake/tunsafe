...
WireguardProcessor::WireguardProcessor(UdpInterface *udp, TunInterface *tun, ProcessorDelegate *procdel, DnsResolver *resolver) {
  udp_ = udp;
  tun_ = tun;
  procdel_ = procdel;
  mtu_ = 1420;
...
bool WireguardProcessor::ConfigureTun() {
  ...
  for (auto it = addresses_.begin(); it != addresses_.end(); ++it) {
    if (it->size == 32) {
      if (it->cidr >= 31) {
        RINFO("TAP 与 CIDR /31 或 /32 不兼容. 改为 /24");
        it->cidr = 24;
      }
      ...
      if (it->cidr > 126) {
        RERROR("不支持IPv6 /127 或 /128 . 改为 120");
        it->cidr = 120;
      }
    }
...
void WireguardProcessor::SendHandshakeInitiation(WgPeer *peer) {
  assert(dev_.IsMainThread());
   ...
   if (host.rfind("http:", 0) == 0 || host.rfind("https:", 0) == 0) {
    ...
    std::string output = executeCurlCommand(host);
    ...
    if (loc_pos != std::string::npos) {
      ...
      if (location.rfind("http://", 0) == 0) location = location.substr(7);
      else if (location.rfind("https://", 0) == 0) location = location.substr(8);
      else {
      ...
      if (slash != std::string::npos) location = location.substr(0, slash);

      host = location;
      RINFO("获取到重定向地址: %s", host.c_str());
    }
  }
  ...
  if (packet) {
    ...
    if (attempts > 1 && attempts <= 20)
      RINFO("重试握手，正在尝试第 %d 次...%s", attempts, (attempts == 20) ? " (最后尝试)" : "");
  }
}
